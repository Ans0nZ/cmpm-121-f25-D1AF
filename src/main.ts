...
// Automatic growth rate (qi per second)
let growthRate = 0;

const UNIT = "qi";
const CLICK_GAIN = 1;

// Step 10: Data-driven design with descriptions
interface Item {
  name: string; // Item name (button title)
  description: string; // Short description (tooltip and small text)
  cost: number; // Base cost
  rate: number; // Qi produced per second
}

// Cultivation-themed items
...
// Calculate rising cost: base price * 1.15 ^ purchases
...
function renderShop(): string {
  return availableItems
    .map(
      (item, index) => `
      <div class="shop-row">
        <button id="buy-${index}" disabled title="${item.description}">
          ...
          <div class="item-cost">
            Cost: <span id="price-${index}">
              ${getCurrentCost(index).toFixed(2)}
            </span> ${UNIT}
          </div>
        </button>
...
// Click meditate: gain qi immediately
meditate.addEventListener("click", () => {
  counter += CLICK_GAIN;
  refreshUI();
});

// Bind purchase buttons dynamically
